...
export default function GameBoard() {
  ...
  function flushBoard() {
    // Using produce for fine-grained updates - only changed properties trigger re-render
    setPieces(
      produce((draft) => {
  ...
  function updateSelection() {
    const sq = gameState.selectedSquare as number;
    if (sq === 0) {
      setSelectedSquare(null);
      setValidMoves([]);
      return;
    ...
    if (piece && piece.pieceType !== 0) {
      ...
      setValidMoves(moves.map((mv) => DST(mv as number)));
    } else {
      setSelectedSquare(null);
      setValidMoves([]);
    }
  ...
  async function animateMove(src: number, dst: number): Promise<void> {
    ...
    return new Promise<void>((resolve) => {
      // Using produce - only the piece at src will trigger re-render
      setPieces(
        produce((draft) => {
  ...
  function checkGameState() {
    ...
    if (engine.inCheck()) {
      computerMove() ? soundManager.playCheck2() : soundManager.playCheck();
    } else if (engine.captured()) {
      computerMove() ? soundManager.playCapture2() : soundManager.playCapture();
    } else {
      computerMove() ? soundManager.playMove2() : soundManager.playMove();
    }
  ...
  onMount(() => {
    setMainScene(() => gameController as any);
    loadGame();
  });
  ...
  return (
    <div
      ref={containerRef}
      class="w-full relative flex justify-center items-center p-1 overflow-hidden"
    >
      <div
        class="game-board w-full max-w-[521px] aspect-[521/577] relative cursor-pointer shadow-2xl rounded-sm"
        style={{
      ...
      >
        ...
        {/* Pieces - using Index for fine-grained updates by index */}
        <Index each={pieces}>
          {(piece) => {
            const textureKey = () =>
              PIECE_IMAGE_MAP[piece().pieceType as keyof typeof PIECE_IMAGE_MAP];
            return (
